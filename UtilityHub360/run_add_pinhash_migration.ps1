# PowerShell script to add PinHash column to Users table
# This script uses the connection string from appsettings.json
# This fixes the "Invalid column name 'PinHash'" error (mobile PIN login)

Write-Host "Adding PinHash column to Users table..." -ForegroundColor Yellow
...
$scriptPath = Join-Path $PSScriptRoot "Scripts\add_pinhash_column_to_users.sql"
...
try {
    ...
    if (Get-Command sqlcmd -ErrorAction SilentlyContinue) {
        ...
        if ($exitCode -eq 0) {
            Write-Host "`nSUCCESS: PinHash column has been added!" -ForegroundColor Green
            if ($output) {
                Write-Host $output
            }
        } else {
            Write-Host "`nsqlcmd failed, trying .NET SqlClient method..." -ForegroundColor Yellow
            $useDotNet = $true
        }
    } else {
