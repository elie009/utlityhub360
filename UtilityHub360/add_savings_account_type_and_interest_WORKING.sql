...
-- WORKING VERSION: Uses GO statements and proper error handling
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'SavingsAccounts')
BEGIN
...
PRINT 'SavingsAccounts table found. Starting migration...';
GO
...
-- Only run if AccountType column exists
IF EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID(N'SavingsAccounts') 
    AND name = 'AccountType'
...
-- Only create if column exists and index doesn't exist
IF EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID(N'SavingsAccounts') 
    AND name = 'NextInterestCalculationDate'
